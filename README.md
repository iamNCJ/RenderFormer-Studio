...
receipts. The CLI records the selected profile, resolved switches, input
preprocessing, and auxiliary component provenance (source, requested revision,
and dtype):
...
The 12 curated RF2-only paper-scene definitions are bundled for the V2 200M
architecture. Generate and postprocess a scene in the datagen environment,
then hand the resulting H5 to the inference launcher in the CUDA environment:
...
The inference launcher selects the official native-512 or native-2048 Hugging
Face model from the scene resolution; set `CHECKPOINT` only to override it. It
does not invoke Blender or generate data.
...
requirements, provenance, and auxiliary-model requirements.

For an RF1 JSON scene, compose the required H5 first:
...
workflow, V1 padding guidance, raw versus encoded V2 inputs, execution
profiles, output manifests, and model version selection. New examples
use hyphenated options; the migrated path, batching, worker, tone-map, and
video arguments also retain their documented legacy underscore aliases.
